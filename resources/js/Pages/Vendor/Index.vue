...
const fetchVendors = async (page = 1) => {
    ...
    const response = await axios.get('/vendor/fetchAllVendors', { params: { page } })
...
const handleSearch = async ({ search, keywords }) => {
    if (!keywords) {
        ...
        vendors.value = await search('/vendor/search');
    }
}
const handleClear = () => {
    fetchVendors();
        ...
        <section class="bg-gray-50 dark:bg-gray-900">
            <div class="mx-auto max-w-full max-h-max">
                <div class="bg-white dark:bg-gray-800 relative shadow-md rounded-xl overflow-hidden">
                    ...
                    <Pagination v-if="vendors.length > 0" :meta="meta" @change-page="fetchVendors" />
                </div>
            </div>
        </section>
    </AppLayout>
