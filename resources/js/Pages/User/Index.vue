...
const fetchUsers = async (page = 1) => {
    ...
    const response = await axios.get('/user/fetchAllUsers', { params: { page } })
...
const handleSearch = async ({ search, keywords }) => {
    if (!keywords) {
        ...
        users.value = await search('/user/search');
    }
}
const handleClear = () => {
    fetchUsers();
...
<template>
    <AppLayout title="NRS | User Management">
        <template #header>
                    ...
                    <div class="overflow-x-auto h-[700px]">
                        <div
                            ...
                            <div
                                ...
                                <PlusSmallIcon class="w-5 h-5 -ml-1 mr-2" /> Add User
                                </Link>
                            </div>
                        </div>
                        ...
                        <table v-else class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="hidden md:table-header-group text-xs text-gray-400 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3 w-[20%]">ID</th>
                                    <th scope="col" class="px-4 py-3 w-[20%]">First Name</th>
                                    <th scope="col" class="px-4 py-3 w-[20%]">Last Name</th>
                                    <th scope="col" class="px-4 py-3 w-[40%]">Email Address</th>
                            ...
                            <tbody>
                                        ...
                                        <div class="flex items-center justify-center h-full">
                                            <div class="flex items-center mt-6 text-center rounded-lg h-96">
                                                <div class="flex flex-col w-full max-w-sm px-4 mx-auto">
                                                    ...
                                                    <h1 class="mt-3 text-lg text-gray-400 dark:text-white">No Users found</h1>
                                ...
                                <tr v-for="(user, index) in users" :key="user.id" class="lg:table-row flex flex-col lg:flex-row border-b dark:border-gray-700">
                                    
                                    <td class="px-4 py-1 lg:py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ user.id }}
                                    </td>
                                    <td class="px-4 py-1 lg:py-">{{ user.name }}</td>
                                    <td class="px-4 py-1 lg:py-3">{{ user.last_name }}</td>
