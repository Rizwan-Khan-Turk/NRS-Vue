...
const fetchAuditLogs = async (page = 1) => {
    ...
    const response = await axios.get('/audit/fetchAllAuditLog', { params: { page } })
    audits.value = response.data.audits.data
...
const handleSearch = async ({ search, keywords }) => {
    if (!keywords) {
        ...
        audits.value = await search('/audit/search');
    }
}
const handleClear = () => {
    fetchAuditLogs();
        ...
        <section class="bg-gray-50 dark:bg-gray-900">
            <div class="mx-auto max-w-full max-h-max">
                <div class="bg-white dark:bg-gray-800 relative shadow-md rounded-xl overflow-hidden">
                        ...
                        <table v-else class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(audit, index) in audits" :key="audit.id" class="lg:table-row flex flex-col lg:flex-row border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-1 lg:py-">{{ audit.vendor }}</td>
                                    <td class="px-4 py-1 lg:py-3">{{ audit.po_number }}</td>
                                    <td class="px-4 py-1 lg:py-3">
                                    {{ audit.transactionType}}
                                    </td>
                                    <td class="px-4 py-1 lg:py-3">{{ audit.created_at }}</td>
                                    <td class="px-4 py-1 lg:py-3">{{ audit.status }}</td>
                                    <td class="px-4 py-4 lg:py-3 flex items-center justify-start lg:justify-end">
                                        <!-- <Link :href="route('pets.show', { slug: audit.slug })"
                                        ...
                                        <button @click="deletePet(audit.id)"
                                            class="inline-flex items-center p-0.5 text-sm font-medium text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100"
                                            type="button">
                    ...
                    <Pagination v-if="audits.length > 0" :meta="meta" @change-page="fetchAuditLogs" />
                </div>
            </div>
        </section>
    </AppLayout>
